...
The package <tt>penalizedclr</tt>  is not limited to L1, i.e. lasso, penalty. While L1 penalty is more suited for variable selection, in the presence of highly correlated covariates, it can be useful to add small amount of  L2 penalty.  The two can be  combined in an elastic net framework by specifying  the mixing parameter `alpha`. Default `alpha=1` gives lasso. 
...
1. <tt>Pilambda</tt> a numeric vector giving estimates of selection probabilities for each penalized covarite.

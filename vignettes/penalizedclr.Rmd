...
The following code performs stability selection when all covariates are considered as a single block (a single data source) and a sequence of L1 penalties contains only 3 values.
...
```{r eval= T}
stable3 <- stable.clr.g(response = Y, p = p,  standardize = TRUE,
                      penalized = X, stratum = stratum,
...
```{r}
# plotting selection probabilities for true non-zero (red) and zero (blue) coefficients
s_prob_nonzero <- cut(stable3$P[nonzero_index == 1], breaks = seq(0, 1, by = 0.1), ordered_result = T)
s_prob_zero <- cut(stable3$P[nonzero_index == 0], breaks = seq(0, 1, by = 0.1), ordered_result = T)
